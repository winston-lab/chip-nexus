...
parser.add_argument('-s', dest = 'bandwidth', type=int, help='bandwidth of Gaussian kernel used to smooth coverage before finding the summit')
...
def average_bigwigs(coverage_paths, bandwidth):
    coverage = {}
    for index, path in enumerate(coverage_paths):
        ...
        for chrom in chroms:
            ...
            if index==len(coverage_paths):
                coverage[chrom] = gaussian_filter1d(np.divide(coverage[chrom], index), bandwidth)
        bw.close()
    return coverage

#given chromosome coordinates,
...
def main(condition_paths, control_paths, bandwidth, diffexp_path, narrowpeak_out, bed_out):
    ...
    condition_coverage = average_bigwigs(condition_paths, bandwidth)
    coverage = average_bigwigs(control_paths, bandwidth)
...
if __name__ == '__main__':
    main(args.condition_paths, args.control_paths, args.bandwidth, args.diffexp_path, args.narrowpeak_out, args.bed_out)
