...
def main():
    ...
    nucleotides = 'ACTGN'
    barcodes_dict = Counter({"".join(nts) : 0 for nts in itertools.product(nucleotides, repeat=5)})
    ligations_dict = Counter({"".join(nts) : 0 for nts in itertools.product(nucleotides, repeat=5)})
    ...
    with open(sys.argv[2], 'w') as out_fastq_file:
        ...
        while header != '':
            ...
            barcode = full_seq[0:5]
            ligation = full_seq[7:13]
            trimmed_seq = full_seq[9:]
            ...
            out_fastq.stdin.write(qual_scores[9:])
            header = in_fastq.stdout.readline().rstrip()
