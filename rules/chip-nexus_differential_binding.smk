...
rule combine_peak_counts:
    input:
        lambda wc: ["diff_binding/{condition}-v-{control}/".format(**wc) + sample + "_{type}-{factor}-chipnexus-peakcounts.tsv".format(**wc) for sample in getsamples(wc.control, wc.condition)]
    output:
...
rule diffbind_results_to_narrowpeak:
    input:
        condition_coverage = lambda wc: expand("coverage/{norm}/{sample}_{factor}-chipnexus-{norm}-midpoints.bw", sample=getsamples(wc.condition, wc.condition), norm=wc.norm, factor=FACTOR),
        control_coverage = lambda wc: expand("coverage/{norm}/{sample}_{factor}-chipnexus-{norm}-midpoints.bw", sample=getsamples(wc.control, wc.control), norm=wc.norm, factor=FACTOR),
